...
server <- function(input, output) {
  ...
  data_deptos <- read.csv('data/departamentos.csv')
  data=data.frame(x=data_deptos$longitud, y=data_deptos$latitud, id=data_deptos$departamento)
  ...
  output$map <- renderLeaflet({
    leaflet() %>% 
      setView(lng=-60.9 , lat =-36, zoom=7) %>%
